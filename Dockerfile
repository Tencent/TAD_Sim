...
RUN apt-get update \
    ...
    liblzma-dev \
    tk-dev \
    # below are the required dependencies when building docker-ce-cli
    apt-transport-https \
    gnupg-agent \
...
# Install cmake
RUN wget -O cmake-linux.sh https://github.com/Kitware/CMake/releases/download/v${CMAKE_VERSION}/cmake-${CMAKE_VERSION}-Linux-x86_64.sh \
    && sh cmake-linux.sh -- --skip-license --prefix=/usr/local \
    && rm cmake-linux.sh \
    && cmake --version

# Install Node.js and npm install node-addon-api
...
# Install ZeroMQ
RUN wget https://github.com/zeromq/libzmq/releases/download/v${ZEROMQ_VERSION}/zeromq-${ZEROMQ_VERSION}.tar.gz \
...
# Install log4cpp
RUN wget https://nchc.dl.sourceforge.net/project/log4cpp/log4cpp-1.1.x%20%28new%29/log4cpp-1.1/log4cpp-${LOG4CPP_VERSION}.tar.gz \
...
# Install pugixml
RUN wget https://github.com/zeux/pugixml/releases/download/v${PUGIXML_VERSION}/pugixml-${PUGIXML_VERSION}.tar.gz \
...
# Install SQLite
RUN wget https://www.sqlite.org/2021/sqlite-amalgamation-${SQLITE_VERSION}.zip \
...
RUN git clone https://github.com/uriparser/uriparser.git \
    && cd uriparser \
    && git checkout uriparser-${URIPARSER_VERSION} \
    && mkdir build \
    && cd build \
...
# Install Chrono
RUN wget https://github.com/projectchrono/chrono/archive/${CHRONO_VERSION}.tar.gz -O projectchrono-chrono-${CHRONO_VERSION}.tar.gz \
    && tar zxf projectchrono-chrono-${CHRONO_VERSION}.tar.gz \
    ...
    && ninja install \
    && cd  ../.. \
    && rm -rf chrono-${CHRONO_VERSION} projectchrono-chrono-${CHRONO_VERSION}.tar.gz

# Install cereal
RUN wget https://github.com/USCiLab/cereal/archive/refs/tags/v${CEREAL_VERSION}.tar.gz \
    && tar zxf v${CEREAL_VERSION}.tar.gz \
    && cd cereal-${CEREAL_VERSION} \
    && cp -r include/cereal /usr/local/include/ \
    && cd .. \
    && rm -rf v${NLOHMANN_VERSION}.tar.gz json-${NLOHMANN_VERSION}

# Install nlohmann JSON
RUN wget https://github.com/nlohmann/json/archive/refs/tags/v${NLOHMANN_VERSION}.tar.gz \
    && tar zxf v${NLOHMANN_VERSION}.tar.gz \
