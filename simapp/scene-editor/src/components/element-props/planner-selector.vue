<template>
  <div class="planner-selector">
    <div class="planner-selector-icon" :title="$getModelName(planner)">
      <div class="planner-selector-avatar">
        <img :src="$getModelIcon(planner, true)" alt="">
      </div>
      <div class="planner-selector-name">
...
export default {
  ...
  computed: {
    ...
    planner () {
      return this.plannerList.find(p => p.variable === this.form.name)
    },
    list () {
  ...
  methods: {
    selectPlanner (planner) {
      this.modifyObjectParams({
        name: planner.variable,
        subType: planner.catalogSubCategory,
      })
      editor.scenario.syncPlannerVisual(this.form.id)
