/* eslint-disable */
import {
  DefaultLoadingManager, LoaderUtils, FileLoader, TextureLoader,
  Loader, RepeatWrapping, ClampToEdgeWrapping, MeshPhongMaterial,
  MeshLambertMaterial, Color, EquirectangularReflectionMapping,
  Matrix4, Math as MathUtils, Euler, Vector3, BufferGeometry,
  Float32BufferAttribute, Uint16BufferAttribute, Matrix3,
  Vector4, BufferAttribute, Group, PropertyBinding, Bone,
  Object3D, PerspectiveCamera, OrthographicCamera, PointLight,
  DirectionalLight, SpotLight, VertexColors, SkinnedMesh,
  LineBasicMaterial, Line, AmbientLight, Quaternion, Skeleton,
  AnimationClip, VectorKeyframeTrack, Mesh, QuaternionKeyframeTrack,
} from 'three'
import { NURBSCurve } from './nurbs-curve'
import { TGALoader } from './tga-loader'
...
function parseMeshGeometry (FBXTree, relationships, geoNode, deformers) {
  ...
  if ('GeometricRotation' in modelNode) {
    var array = modelNode.GeometricRotation.value.map(MathUtils.degToRad)
    array[3] = 'ZYX'
...
function parseNurbsGeometry (geoNode) {
  ...
  if (geoNode.Form === 'Closed') {
    ...
    for (let j = 0; j < degree; ++j) {
      controlPoints.push(controlPoints[j])
    }
  }
...
function setModelTransforms (FBXTree, model, modelNode) {
  ...
  if ('Lcl_Rotation' in modelNode) {
    var rotation = modelNode.Lcl_Rotation.value.map(MathUtils.degToRad)
    rotation.push('ZYX')
  ...
  if ('PreRotation' in modelNode) {
    var array = modelNode.PreRotation.value.map(MathUtils.degToRad)
    array[3] = 'ZYX'
...
function generateRotationTrack (modelName, curves, initialValue, preRotations) {
  ...
  if (preRotations !== undefined) {
    preRotations = preRotations.map(MathUtils.degToRad)
...
Object.assign(BinaryParser.prototype, {
  ...
  parseNode: function (reader, version) {
    console.log(reader, version, 222222)
    var node = {}
