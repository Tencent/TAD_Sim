 ...
 * @param params
 */
export function moveCrosswalk (params: {
  objectId: string
  point?: common.vec3
  s?: number
}) {
  ...
  let crosswalkAttr
  if (point) {
    crosswalkAttr = getCrosswalkAttrOnRoad({
      objectId,
      roadId,
      point,
      crossBorder: true,
    ...
    crosswalkAttr = getCrosswalkAttrOnRoadByST({
      objectId,
      roadId,
      s,
    })
  ...
  roadSignData.length = fixedPrecision(
    length || Constant.crosswalkSize.length.default,
  )
 ...
 * @param params
 */
export function getCrosswalkAttrOnRoadByST (params: {
  objectId: string
  roadId: string
  s: number
}) {
...
// 获取人行横道在道路某一处位置的属性
export function getCrosswalkAttrOnRoad (params: {
  objectId?: string
  roadId: string
  point: common.vec3
  ...
  if (crossBorder) {
    ...
    const range = Constant.crosswalkSize.offset + objectData.width / 2
...
// 创建人行横道的数据结构
export function createCrosswalk (params: {
  ...
  roadSignData.width = Constant.crosswalkSize.width.default
