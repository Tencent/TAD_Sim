...
// we need include the two corresponding protobuf headers
// here. if you need process other type of protobuf message that TADSim defines, include them too.
#include <string>
...
MyModule::~MyModule() { std::cout << "MyModule destroyed." << std::endl; }
...
void MyModule::Init(tx_sim::InitHelper& helper) {
  ...
  int argc = 1;
  std::string name = "ros_adapter";  // ROS节点名称
  char* arg = "ros_adapter";
  ros::init(argc, arg, "listener");

  m_ros_node_handle.subscribe("chatter", 1000, chatterCallback);
};
...
void MyModule::Step(tx_sim::StepHelper& helper) {
  ...
  std::cout << std::fixed << std::setprecision(12) << "received location: x -> " << cur_x_ << " y -> " << cur_y_
            << std::endl;
  ...
  ros::spinOnce();
  // print ros message;
  std::cout << "ros message: " << m_ros_msg->data.c_str() << std::endl;
  if (puber_) {
    ...
    // since the message we want to publish is a protobuf message type and the PublishMessage API only accepts the
    // std::string type, we need serialize it to std::string using google::protobuf::MessageLite::SerializeToString
    // method.
    ...
    std::cout << tx_sim::topic::kTraffic << ": " << traffic.cars_size() << " cars, " << traffic.staticobstacles_size()
              << " static obstacles, " << traffic.dynamicobstacles_size() << " dynamic obstacles, "
...
void MyModule::chatterCallback(const std_msgs::String::ConstPtr& msg) { m_ros_msg = msg; }
