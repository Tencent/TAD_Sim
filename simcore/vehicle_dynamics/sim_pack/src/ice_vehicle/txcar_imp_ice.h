...
class TxCarImp_ICE;
using TxCarImpPtr = std::shared_ptr<TxCarImp_ICE>;

/**
 * @brief txcar implementation
 */
class TxCarImp_ICE final : public TxCarBase {
 public:
  TxCarImp_ICE();
  virtual ~TxCarImp_ICE();
 ...
 protected:
  ...
  // msg convert
  void controlToVehicle(ExtU_TxCar_ICE_T &car_u, const std::string &ctrl_payload);
  void controlV2ToVehicle(ExtU_TxCar_ICE_T &car_u, const std::string &ctrl_payload);
  void vehicleToChassis(double t_ms, const ExtU_TxCar_ICE_T &car_u, const ExtY_TxCar_ICE_T &car_y,
                        std::string &chassis_payload);
  void vehicleToLocation(double t_ms, const ExtY_TxCar_ICE_T &car_y, std::string &loc_payload);
