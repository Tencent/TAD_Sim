 ...
 *  -------------------------------------------------------------------------
 *
 * Created: Wed Dec 06 16:03:44 2023
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME engine_sfunction
...
/* Input Port  0 */
#define IN_PORT_0_NAME EngineIn
...
#define IN_0_BUS_NAME ENGINE_INPUTBUS
...
/* Output Port  0 */
#define OUT_PORT_0_NAME EngineOut
...
#define OUT_0_BUS_NAME ENGINE_OUTPUTBUS
...
#define SOURCEFILES                                                                               \
  ...
  "../../inc/r2022b__SFB__INC_PATH ../../deps/__SFB__INC_PATH ../../src/engine__SFB__INC_PATH ../../__SFB__tx_car.lib"
...
static void mdlInitializeSizes(SimStruct *S) {
  ...
  if (!ssSetNumInputPorts(S, NUM_INPUTS)) return;
    ...
    /* Register ENGINE_INPUTBUS datatype for Input port 0 */
#if defined(MATLAB_MEX_FILE)

  if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
    DTypeId dataTypeIdReg;
    ssRegisterTypeFromNamedObject(S, "ENGINE_INPUTBUS", &dataTypeIdReg);
  ...
  if (!ssSetNumOutputPorts(S, NUM_OUTPUTS)) return;
    ...
    /* Register ENGINE_OUTPUTBUS datatype for Output port 0 */
#if defined(MATLAB_MEX_FILE)

  if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
    DTypeId dataTypeIdReg;
    ssRegisterTypeFromNamedObject(S, "ENGINE_OUTPUTBUS", &dataTypeIdReg);
    if (dataTypeIdReg == INVALID_DTYPE_ID) return;
  ...
  ssSetBusOutputObjectName(S, 0, (void *)"ENGINE_OUTPUTBUS");
  ...
  if (!ssSetNumDWork(S, 2)) return;
    ...
    /*
     * Configure the dwork 0 (EngineInBUS)
     */
#if defined(MATLAB_MEX_FILE)
  ...
  ssSetDWorkName(S, 0, "EngineInBUS");
  ssSetDWorkWidth(S, 0, DYNAMICALLY_SIZED);
  ssSetDWorkComplexSignal(S, 0, COMPLEX_NO);

  /*
   * Configure the dwork 1 (EngineOutBUS)
   */
#if defined(MATLAB_MEX_FILE)
  ...
  ssSetDWorkName(S, 1, "EngineOutBUS");
...
static void mdlStart(SimStruct *S) {
  ...
  DTypeId ENGINE_INPUTBUSId = ssGetDataTypeId(S, "ENGINE_INPUTBUS");
  DTypeId ENGINE_OUTPUTBUSId = ssGetDataTypeId(S, "ENGINE_OUTPUTBUS");
  ...
  busInfo[0].offset = dtaGetDataTypeElementOffset(dta, bpath, ENGINE_INPUTBUSId, 0);
  busInfo[0].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[0].numElems = 1;
  busInfo[1].offset = dtaGetDataTypeElementOffset(dta, bpath, ENGINE_INPUTBUSId, 1);
  busInfo[1].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[1].numElems = 1;
  busInfo[2].offset = dtaGetDataTypeElementOffset(dta, bpath, ENGINE_OUTPUTBUSId, 0);
  busInfo[2].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[2].numElems = 1;
  busInfo[3].offset = dtaGetDataTypeElementOffset(dta, bpath, ENGINE_OUTPUTBUSId, 1);
  busInfo[3].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[3].numElems = 1;
  busInfo[4].offset = dtaGetDataTypeElementOffset(dta, bpath, ENGINE_OUTPUTBUSId, 2);
  busInfo[4].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[4].numElems = 1;
  busInfo[5].offset = dtaGetDataTypeElementOffset(dta, bpath, ENGINE_OUTPUTBUSId, 3);
  busInfo[5].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[5].numElems = 1;
  busInfo[6].elemSize = dtaGetDataTypeSize(dta, bpath, ENGINE_INPUTBUSId);
  busInfo[6].numElems = ssGetInputPortWidth(S, 0);
  busInfo[7].elemSize = dtaGetDataTypeSize(dta, bpath, ENGINE_OUTPUTBUSId);
...
static void mdlOutputs(SimStruct *S, int_T tid) {
  const char *EngineIn = (char *)ssGetInputPortSignal(S, 0);
  char *EngineOut = (char *)ssGetOutputPortSignal(S, 0);
  const int_T p_width0 = mxGetNumberOfElements(PARAM_DEF0(S));
  const int_T p_width1 = mxGetNumberOfElements(PARAM_DEF1(S));
  const int8_T *lic_path = (const int8_T *)mxGetData(PARAM_DEF0(S));
  const int8_T *par_path = (const int8_T *)mxGetData(PARAM_DEF1(S));
  ...
  ENGINE_INPUTBUS _EngineInBUS;
  ENGINE_OUTPUTBUS _EngineOutBUS;
  slDataTypeAccess *dta = ssGetDataTypeAccess(S);
  const char *bpath = ssGetPath(S);

  /*Copy from Simulink to bus structure*/

  /*Copy from Simulink to _EngineInBUS for Input port 0*/
  (_EngineInBUS).throttle_01 = *((real_T *)((char *)EngineIn + busInfo[0].offset));
  (_EngineInBUS).eng_spd_rpm = *((real_T *)((char *)EngineIn + busInfo[1].offset));
  engine_sfunction_Outputs_wrapper(&_EngineInBUS, &_EngineOutBUS, lic_path, p_width0, par_path, p_width1);

  /*Copy from bus structure to Simulink*/

  /*Copy from _EngineOutBUS to Simulink for Output port 0*/
  *((real_T *)((char *)EngineOut + busInfo[2].offset)) = (_EngineOutBUS).eng_trq_Nm;
  *((real_T *)((char *)EngineOut + busInfo[3].offset)) = (_EngineOutBUS).pow_out_W;
  *((real_T *)((char *)EngineOut + busInfo[4].offset)) = (_EngineOutBUS).fuel_rate_g_s;
  *((real_T *)((char *)EngineOut + busInfo[5].offset)) = (_EngineOutBUS).fuel_cosume_L;
}
