/*
 * File: front_motor_sfunction.cpp
 *
 *
 ...
 *  -------------------------------------------------------------------------
 *
 * Created: Thu Jul 27 19:38:47 2023
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME front_motor_sfunction
...
#define OUT_0_BUS_NAME motor_out
...
#define SOURCEFILES                                                                               \
  ...
  "../../inc/r2022b__SFB__INC_PATH ../../deps/__SFB__INC_PATH ../../src/motor__SFB__INC_PATH ../../__SFB__tx_car.lib"
...
static void mdlInitializeSizes(SimStruct *S) {
  ...
  if (!ssSetNumInputPorts(S, NUM_INPUTS)) return;
    ...
    /* Register moter_in datatype for Input port 0 */
#if defined(MATLAB_MEX_FILE)

  if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
    DTypeId dataTypeIdReg;
    ssRegisterTypeFromNamedObject(S, "moter_in", &dataTypeIdReg);
  ...
  if (!ssSetNumOutputPorts(S, NUM_OUTPUTS)) return;
    ...
    /* Register motor_out datatype for Output port 0 */
#if defined(MATLAB_MEX_FILE)

  if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
    DTypeId dataTypeIdReg;
    ssRegisterTypeFromNamedObject(S, "motor_out", &dataTypeIdReg);
    if (dataTypeIdReg == INVALID_DTYPE_ID) return;
  ...
  ssSetBusOutputObjectName(S, 0, (void *)"motor_out");
  ...
  if (!ssSetNumDWork(S, 2)) return;
    ...
    /*
     * Configure the dwork 0 (MotorInBUS)
     */
#if defined(MATLAB_MEX_FILE)
  ...
  ssSetDWorkName(S, 0, "MotorInBUS");
  ssSetDWorkWidth(S, 0, DYNAMICALLY_SIZED);
  ssSetDWorkComplexSignal(S, 0, COMPLEX_NO);

  /*
   * Configure the dwork 1 (MotorOutBUS)
   */
#if defined(MATLAB_MEX_FILE)
  ...
  ssSetDWorkName(S, 1, "MotorOutBUS");
...
static void mdlStart(SimStruct *S) {
  ...
  DTypeId moter_inId = ssGetDataTypeId(S, "moter_in");
  DTypeId motor_outId = ssGetDataTypeId(S, "motor_out");
  busInfoStruct *busInfo = (busInfoStruct *)malloc(7 * sizeof(busInfoStruct));
  if (busInfo == NULL) {
  ...
  busInfo[0].offset = dtaGetDataTypeElementOffset(dta, bpath, moter_inId, 0);
  busInfo[0].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[0].numElems = 1;
  busInfo[1].offset = dtaGetDataTypeElementOffset(dta, bpath, moter_inId, 1);
  busInfo[1].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[1].numElems = 1;
  busInfo[2].offset = dtaGetDataTypeElementOffset(dta, bpath, moter_inId, 2);
  busInfo[2].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[2].numElems = 1;
  busInfo[3].offset = dtaGetDataTypeElementOffset(dta, bpath, motor_outId, 0);
  busInfo[3].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[3].numElems = 1;
  busInfo[4].offset = dtaGetDataTypeElementOffset(dta, bpath, motor_outId, 1);
  busInfo[4].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[4].numElems = 1;
  busInfo[5].elemSize = dtaGetDataTypeSize(dta, bpath, moter_inId);
  busInfo[5].numElems = 1;
  busInfo[6].elemSize = dtaGetDataTypeSize(dta, bpath, motor_outId);
...
static void mdlOutputs(SimStruct *S, int_T tid) {
  const char *MotorIn = (char *)ssGetInputPortSignal(S, 0);
  char *MotorOut = (char *)ssGetOutputPortSignal(S, 0);
  const int_T p_width0 = mxGetNumberOfElements(PARAM_DEF0(S));
  const int_T p_width1 = mxGetNumberOfElements(PARAM_DEF1(S));
  const int8_T *lic_path = (const int8_T *)mxGetData(PARAM_DEF0(S));
  const int8_T *par_path = (const int8_T *)mxGetData(PARAM_DEF1(S));
  ...
  moter_in _MotorInBUS;
  motor_out _MotorOutBUS;
  slDataTypeAccess *dta = ssGetDataTypeAccess(S);
  const char *bpath = ssGetPath(S);

  /*Copy from Simulink to bus structure*/

  /*Copy from Simulink to _MotorInBUS for Input port 0*/
  (_MotorInBUS).trq_cmd_Nm = *((real_T *)((char *)MotorIn + busInfo[0].offset));
  (_MotorInBUS).mot_spd_RPM = *((real_T *)((char *)MotorIn + busInfo[1].offset));
  (_MotorInBUS).batt_volt_V = *((real_T *)((char *)MotorIn + busInfo[2].offset));
  front_motor_sfunction_Outputs_wrapper(&_MotorInBUS, &_MotorOutBUS, lic_path, p_width0, par_path, p_width1);

  /*Copy from bus structure to Simulink*/

  /*Copy from _MotorOutBUS to Simulink for Output port 0*/
  *((real_T *)((char *)MotorOut + busInfo[3].offset)) = (_MotorOutBUS).batt_curr_A;
  *((real_T *)((char *)MotorOut + busInfo[4].offset)) = (_MotorOutBUS).mot_trq_Nm;
}
