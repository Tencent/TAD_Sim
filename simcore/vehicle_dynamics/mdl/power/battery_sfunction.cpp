 ...
 *  -------------------------------------------------------------------------
 *
 * Created: Thu Jul 27 19:39:31 2023
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME battery_sfunction
...
/* Output Port  0 */
#define OUT_PORT_0_NAME BattOut
...
#define OUT_0_BUS_NAME batt_out
...
#define SOURCEFILES                                                                               \
  ...
  "../../inc/r2022b__SFB__INC_PATH ../../deps/__SFB__INC_PATH ../../src/battery__SFB__INC_PATH "  \
  "../../__SFB__tx_car.lib"
...
static void mdlInitializeSizes(SimStruct *S) {
  ...
  if (!ssSetNumInputPorts(S, NUM_INPUTS)) return;
    ...
    /* Register batt_in datatype for Input port 0 */
#if defined(MATLAB_MEX_FILE)

  if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
    DTypeId dataTypeIdReg;
    ssRegisterTypeFromNamedObject(S, "batt_in", &dataTypeIdReg);
  ...
  if (!ssSetNumOutputPorts(S, NUM_OUTPUTS)) return;
    ...
    /* Register batt_out datatype for Output port 0 */
#if defined(MATLAB_MEX_FILE)

  if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
    DTypeId dataTypeIdReg;
    ssRegisterTypeFromNamedObject(S, "batt_out", &dataTypeIdReg);
    if (dataTypeIdReg == INVALID_DTYPE_ID) return;
  ...
  ssSetBusOutputObjectName(S, 0, (void *)"batt_out");
  ...
  if (!ssSetNumDWork(S, 2)) return;
    ...
    /*
     * Configure the dwork 0 (BattInBUS)
     */
#if defined(MATLAB_MEX_FILE)
  ...
  ssSetDWorkName(S, 0, "BattInBUS");
  ssSetDWorkWidth(S, 0, DYNAMICALLY_SIZED);
  ssSetDWorkComplexSignal(S, 0, COMPLEX_NO);

  /*
   * Configure the dwork 1 (BattOutBUS)
   */
#if defined(MATLAB_MEX_FILE)
...
static void mdlStart(SimStruct *S) {
  ...
  DTypeId batt_inId = ssGetDataTypeId(S, "batt_in");
  DTypeId batt_outId = ssGetDataTypeId(S, "batt_out");
  ...
  busInfo[0].offset = dtaGetDataTypeElementOffset(dta, bpath, batt_inId, 0);
  busInfo[0].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[0].numElems = 1;
  busInfo[1].offset = dtaGetDataTypeElementOffset(dta, bpath, batt_inId, 1);
  busInfo[1].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[1].numElems = 1;
  busInfo[2].offset = dtaGetDataTypeElementOffset(dta, bpath, batt_outId, 0);
  busInfo[2].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[2].numElems = 1;
  busInfo[3].offset = dtaGetDataTypeElementOffset(dta, bpath, batt_outId, 1);
  busInfo[3].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[3].numElems = 1;
  busInfo[4].offset = dtaGetDataTypeElementOffset(dta, bpath, batt_outId, 2);
  busInfo[4].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[4].numElems = 1;
  busInfo[5].offset = dtaGetDataTypeElementOffset(dta, bpath, batt_outId, 3);
  busInfo[5].elemSize = dtaGetDataTypeSize(dta, bpath, SS_DOUBLE);
  busInfo[5].numElems = 1;
  busInfo[6].elemSize = dtaGetDataTypeSize(dta, bpath, batt_inId);
  busInfo[6].numElems = 1;
  busInfo[7].elemSize = dtaGetDataTypeSize(dta, bpath, batt_outId);
...
static void mdlOutputs(SimStruct *S, int_T tid) {
  const char *BattIn = (char *)ssGetInputPortSignal(S, 0);
  char *BattOut = (char *)ssGetOutputPortSignal(S, 0);
  const int_T p_width0 = mxGetNumberOfElements(PARAM_DEF0(S));
  const int_T p_width1 = mxGetNumberOfElements(PARAM_DEF1(S));
  const int8_T *lic_path = (const int8_T *)mxGetData(PARAM_DEF0(S));
  const int8_T *par_path = (const int8_T *)mxGetData(PARAM_DEF1(S));
  ...
  batt_in _BattInBUS;
  batt_out _BattOutBUS;
  slDataTypeAccess *dta = ssGetDataTypeAccess(S);
  const char *bpath = ssGetPath(S);

  /*Copy from Simulink to bus structure*/

  /*Copy from Simulink to _BattInBUS for Input port 0*/
  (_BattInBUS).batt_curr_A = *((real_T *)((char *)BattIn + busInfo[0].offset));
  (_BattInBUS).batt_temp_K = *((real_T *)((char *)BattIn + busInfo[1].offset));
  battery_sfunction_Outputs_wrapper(&_BattInBUS, &_BattOutBUS, lic_path, p_width0, par_path, p_width1);

  /*Copy from bus structure to Simulink*/

  /*Copy from _BattOutBUS to Simulink for Output port 0*/
  *((real_T *)((char *)BattOut + busInfo[2].offset)) = (_BattOutBUS).batt_volt_V;
  *((real_T *)((char *)BattOut + busInfo[3].offset)) = (_BattOutBUS).batt_soc_null;
  *((real_T *)((char *)BattOut + busInfo[4].offset)) = (_BattOutBUS).batt_pwr_W;
  *((real_T *)((char *)BattOut + busInfo[5].offset)) = (_BattOutBUS).batt_loss_pwr_W;
}
