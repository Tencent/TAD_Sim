...
#include "eval_pred_heading_avg.h"
#include "trajectory.pb.h"
#include "utils/proto_helper.h"

namespace eval {
const char EvalPredHeadingAvg::_kpi_name[] = "Prediction_HeadingErrorAverage";

sim_msg::TestReport_XlsxSheet EvalPredHeadingAvg::s_lateral_sheet;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredHeadingAvg::s_index;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredHeadingAvg::s_t;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredHeadingAvg::s_fellow_id;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredHeadingAvg::s_error;

EvalPredHeadingAvg::EvalPredHeadingAvg() { VLOG_0 << "eval algorithm " << _kpi_name << " constructed.\n"; }
bool EvalPredHeadingAvg::Init(eval::EvalInit &helper) {
  ...
  if (isReportEnabled()) {
    ...
    ReportHelper::ConfigSheetData(s_error, "average heading error[rad]");
    ReportHelper::ConfigXLSXSheet(s_lateral_sheet, "average prediction heading error", "");
  }
...
bool EvalPredHeadingAvg::Step(eval::EvalStep &helper) {
    ...
    for (auto fellow_info : fellow_infos) {
      ...
      for (auto i = 0; fellow_info && i < fellow_info->m_actual_traj.size(); ++i) {
        ...
        if (0 <= index && index < fellow_info->m_actual_traj.size()) {
          ...
          // sum error
          avg_error +=
              std::abs(CEvalMath::YawDiff(actual_pt->GetLocation().GetEuler(), pred_pt.GetLocation().GetEuler()));
          counter++;
        }
      }
...
bool EvalPredHeadingAvg::Stop(eval::EvalStop &helper) {
  // add report
  if (isReportEnabled()) {
...
void EvalPredHeadingAvg::SetGradingMsg(sim_msg::Grading &msg) {
  // set detected event
  EvalHelper::SetDetectedEvent(msg, _detector, _kpi_name);
}

EvalResult EvalPredHeadingAvg::IsEvalPass() {
  if (m_KpiEnabled) {
...
bool EvalPredHeadingAvg::ShouldStopScenario(std::string &reason) {
