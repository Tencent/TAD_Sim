...
#include "eval_pred_vertical_avg.h"
#include "trajectory.pb.h"
#include "utils/proto_helper.h"

namespace eval {
const char EvalPredVerticalAvg::_kpi_name[] = "Prediction_VerticalErrorAverage";

sim_msg::TestReport_XlsxSheet EvalPredVerticalAvg::s_lateral_sheet;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredVerticalAvg::s_index;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredVerticalAvg::s_t;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredVerticalAvg::s_fellow_id;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredVerticalAvg::s_error;

EvalPredVerticalAvg::EvalPredVerticalAvg() { VLOG_0 << "eval algorithm " << _kpi_name << " constructed.\n"; }
bool EvalPredVerticalAvg::Init(eval::EvalInit &helper) {
  ...
  if (isReportEnabled()) {
    ...
    ReportHelper::ConfigSheetData(s_error, "average vertical error[m]");
    ReportHelper::ConfigXLSXSheet(s_lateral_sheet, "average prediction vertical error", "");
  }
...
bool EvalPredVerticalAvg::Step(eval::EvalStep &helper) {
    ...
    for (auto fellow_info : fellow_infos) {
      ...
      for (auto i = 0; fellow_info && i < fellow_info->m_actual_traj.size(); ++i) {
        ...
        if (0 <= index && index < fellow_info->m_actual_traj.size()) {
          ...
          // sum error
          avg_error += std::abs(CEvalMath::GetVerticalDist(actual_pt->GetLocation(), pred_pt.GetLocation()));
          counter++;
        }
      }
...
bool EvalPredVerticalAvg::Stop(eval::EvalStop &helper) {
  // add report
  if (isReportEnabled()) {
...
void EvalPredVerticalAvg::SetGradingMsg(sim_msg::Grading &msg) {
  // set detected event
  EvalHelper::SetDetectedEvent(msg, _detector, _kpi_name);
}

EvalResult EvalPredVerticalAvg::IsEvalPass() {
  if (m_KpiEnabled) {
    _case.mutable_info()->set_detected_count(_detector.GetCount());
    if (_detector.GetCount() >= m_Kpi.passcondition().value() && m_Kpi.passcondition().value() >= 0.5) {
      return EvalResult(sim_msg::TestReport_TestState_FAIL, "above prediction vertical error");
    } else {
      return EvalResult(sim_msg::TestReport_TestState_PASS, "prediction vertical error check pass");
    }
  }

  return EvalResult(sim_msg::TestReport_TestState_PASS, "prediction vertical error check skipped");
}
bool EvalPredVerticalAvg::ShouldStopScenario(std::string &reason) {
  auto ret = _detector.GetCount() >= m_Kpi.finishcondition().value() && m_Kpi.finishcondition().value() >= 0.5;
  if (ret) reason = "above prediction vertical error";
