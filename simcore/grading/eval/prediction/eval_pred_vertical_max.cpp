...
#include "eval_pred_vertical_max.h"
#include "trajectory.pb.h"
#include "utils/proto_helper.h"

namespace eval {
const char EvalPredVerticalMax::_kpi_name[] = "Prediction_VerticalErrorMax";

sim_msg::TestReport_XlsxSheet EvalPredVerticalMax::s_lateral_sheet;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredVerticalMax::s_index;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredVerticalMax::s_t;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredVerticalMax::s_fellow_id;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredVerticalMax::s_error;

EvalPredVerticalMax::EvalPredVerticalMax() { VLOG_0 << "eval algorithm " << _kpi_name << " constructed.\n"; }
bool EvalPredVerticalMax::Init(eval::EvalInit &helper) {
  ...
  if (isReportEnabled()) {
    ...
    ReportHelper::ConfigSheetData(s_error, "max vertical error[m]");
    ReportHelper::ConfigXLSXSheet(s_lateral_sheet, "max prediction vertical error", "");
  }
...
bool EvalPredVerticalMax::Step(eval::EvalStep &helper) {
      ...
      for (auto i = 0; fellow_info && i < fellow_info->m_actual_traj.size(); ++i) {
        ...
        if (0 <= index && index < fellow_info->m_actual_traj.size()) {
          ...
          max_error = std::max(max_error,
                               std::abs(CEvalMath::GetVerticalDist(actual_pt->GetLocation(), pred_pt.GetLocation())));
        }
      }
...
bool EvalPredVerticalMax::Stop(eval::EvalStop &helper) {
  // add report
  if (isReportEnabled()) {
...
void EvalPredVerticalMax::SetGradingMsg(sim_msg::Grading &msg) {
  // set detected event
  EvalHelper::SetDetectedEvent(msg, _detector, _kpi_name);
}

EvalResult EvalPredVerticalMax::IsEvalPass() {
  if (m_KpiEnabled) {
...
bool EvalPredVerticalMax::ShouldStopScenario(std::string &reason) {
