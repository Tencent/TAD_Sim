...
#include "eval_pred_lateral_avg.h"
#include "trajectory.pb.h"
#include "utils/proto_helper.h"

namespace eval {
const char EvalPredLateralAvg::_kpi_name[] = "Prediction_LateralErrorAverage";

sim_msg::TestReport_XlsxSheet EvalPredLateralAvg::s_lateral_sheet;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredLateralAvg::s_index;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredLateralAvg::s_t;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredLateralAvg::s_fellow_id;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredLateralAvg::s_error;

EvalPredLateralAvg::EvalPredLateralAvg() { VLOG_0 << "eval algorithm " << _kpi_name << " constructed.\n"; }
bool EvalPredLateralAvg::Init(eval::EvalInit &helper) {
  ...
  if (isReportEnabled()) {
    ...
    ReportHelper::ConfigSheetData(s_error, "average lateral error[m]");
    ReportHelper::ConfigXLSXSheet(s_lateral_sheet, "average prediction lateral error", "");
  }
...
bool EvalPredLateralAvg::Step(eval::EvalStep &helper) {
    ...
    for (auto fellow_info : fellow_infos) {
      ...
      for (auto i = 0; fellow_info && i < fellow_info->m_actual_traj.size(); ++i) {
        ...
        if (0 <= index && index < fellow_info->m_actual_traj.size()) {
          ...
          // sum error
          avg_error += std::abs(CEvalMath::GetLateralDist(actual_pt->GetLocation(), pred_pt.GetLocation()));
          counter++;
        }
      }
...
bool EvalPredLateralAvg::Stop(eval::EvalStop &helper) {
  // add report
  if (isReportEnabled()) {
...
void EvalPredLateralAvg::SetGradingMsg(sim_msg::Grading &msg) {
  // set detected event
  EvalHelper::SetDetectedEvent(msg, _detector, _kpi_name);
}

EvalResult EvalPredLateralAvg::IsEvalPass() {
  if (m_KpiEnabled) {
    _case.mutable_info()->set_detected_count(_detector.GetCount());
    if (_detector.GetCount() >= m_Kpi.passcondition().value() && m_Kpi.passcondition().value() >= 0.5) {
      return EvalResult(sim_msg::TestReport_TestState_FAIL, "above prediction lateral error");
    } else {
      return EvalResult(sim_msg::TestReport_TestState_PASS, "prediction lateral error check pass");
    }
  }

  return EvalResult(sim_msg::TestReport_TestState_PASS, "prediction lateral error check skipped");
}
bool EvalPredLateralAvg::ShouldStopScenario(std::string &reason) {
  auto ret = _detector.GetCount() >= m_Kpi.finishcondition().value() && m_Kpi.finishcondition().value() >= 0.5;
  if (ret) reason = "above prediction lateral error";
