...
#include "eval_pred_lateral_max.h"
#include "trajectory.pb.h"
#include "utils/proto_helper.h"

namespace eval {
const char EvalPredLateralMax::_kpi_name[] = "Prediction_LateralErrorMax";

sim_msg::TestReport_XlsxSheet EvalPredLateralMax::s_lateral_sheet;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredLateralMax::s_index;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredLateralMax::s_t;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredLateralMax::s_fellow_id;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredLateralMax::s_error;

EvalPredLateralMax::EvalPredLateralMax() { VLOG_0 << "eval algorithm " << _kpi_name << " constructed.\n"; }
bool EvalPredLateralMax::Init(eval::EvalInit &helper) {
  ...
  if (isReportEnabled()) {
    ...
    ReportHelper::ConfigSheetData(s_error, "max lateral error[m]");
    ReportHelper::ConfigXLSXSheet(s_lateral_sheet, "max prediction lateral error", "");
  }
...
bool EvalPredLateralMax::Step(eval::EvalStep &helper) {
    ...
    for (auto fellow_info : fellow_infos) {
      ...
      for (auto i = 0; fellow_info && i < fellow_info->m_actual_traj.size(); ++i) {
        ...
        if (0 <= index && index < fellow_info->m_actual_traj.size()) {
          ...
          // sum error
          max_error =
              std::max(max_error, std::abs(CEvalMath::GetLateralDist(actual_pt->GetLocation(), pred_pt.GetLocation())));
        }
      }
...
bool EvalPredLateralMax::Stop(eval::EvalStop &helper) {
  // add report
  if (isReportEnabled()) {
...
void EvalPredLateralMax::SetGradingMsg(sim_msg::Grading &msg) {
  // set detected event
  EvalHelper::SetDetectedEvent(msg, _detector, _kpi_name);
}

EvalResult EvalPredLateralMax::IsEvalPass() {
  if (m_KpiEnabled) {
...
bool EvalPredLateralMax::ShouldStopScenario(std::string &reason) {
