...
#include "eval_pred_acc_max.h"
#include "trajectory.pb.h"
#include "utils/proto_helper.h"

namespace eval {
const char EvalPredAccMax::_kpi_name[] = "Prediction_AccErrorMax";

sim_msg::TestReport_XlsxSheet EvalPredAccMax::s_lateral_sheet;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredAccMax::s_index;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredAccMax::s_t;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredAccMax::s_fellow_id;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredAccMax::s_error;

EvalPredAccMax::EvalPredAccMax() { VLOG_0 << "eval algorithm " << _kpi_name << " constructed.\n"; }
bool EvalPredAccMax::Init(eval::EvalInit &helper) {
  ...
  if (isReportEnabled()) {
    ...
    ReportHelper::ConfigSheetData(s_error, "max acc error[m/s2]");
    ReportHelper::ConfigXLSXSheet(s_lateral_sheet, "max prediction acc error", "");
  }
...
bool EvalPredAccMax::Step(eval::EvalStep &helper) {
  ...
  if (IsModuleValid() && m_KpiEnabled) {
    ...
    if (traffic_msg.GetPayload().size() > 0 && m_fellow_payload != traffic_msg.GetPayload()) {
    ...
    for (auto fellow_info : fellow_infos) {
      ...
      for (auto i = 0; fellow_info && i < fellow_info->m_actual_traj.size(); ++i) {
        ...
        if (0 <= index && index < fellow_info->m_actual_traj.size()) {
          ...
          // sum error
          max_error = std::max(max_error, std::abs(actual_pt->GetAcc().GetX() - pred_pt.GetAcc().GetX()));
        }
      }
...
bool EvalPredAccMax::Stop(eval::EvalStop &helper) {
  // add report
  if (isReportEnabled()) {
...
void EvalPredAccMax::SetGradingMsg(sim_msg::Grading &msg) {
  // set detected event
  EvalHelper::SetDetectedEvent(msg, _detector, _kpi_name);
}

EvalResult EvalPredAccMax::IsEvalPass() {
  if (m_KpiEnabled) {
...
bool EvalPredAccMax::ShouldStopScenario(std::string &reason) {
