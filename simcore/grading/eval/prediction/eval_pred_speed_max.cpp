...
#include "eval_pred_speed_max.h"
#include "trajectory.pb.h"
#include "utils/proto_helper.h"

namespace eval {
const char EvalPredSpeedMax::_kpi_name[] = "Prediction_SpeedErrorMax";

sim_msg::TestReport_XlsxSheet EvalPredSpeedMax::s_lateral_sheet;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredSpeedMax::s_index;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredSpeedMax::s_t;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredSpeedMax::s_fellow_id;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredSpeedMax::s_error;

EvalPredSpeedMax::EvalPredSpeedMax() { VLOG_0 << "eval algorithm " << _kpi_name << " constructed.\n"; }
bool EvalPredSpeedMax::Init(eval::EvalInit &helper) {
  ...
  if (isReportEnabled()) {
    ...
    ReportHelper::ConfigSheetData(s_error, "max speed error[m]");
    ReportHelper::ConfigXLSXSheet(s_lateral_sheet, "max prediction speed error", "");
  }
...
bool EvalPredSpeedMax::Step(eval::EvalStep &helper) {
    ...
    for (auto fellow_info : fellow_infos) {
      ...
      for (auto i = 0; fellow_info && i < fellow_info->m_actual_traj.size(); ++i) {
        ...
        if (0 <= index && index < fellow_info->m_actual_traj.size()) {
          ...
          // sum error
          max_error =
              std::max(max_error, std::abs(actual_pt->GetSpeed().GetNormal2D() - pred_pt.GetSpeed().GetNormal2D()));
        }
      }
...
bool EvalPredSpeedMax::Stop(eval::EvalStop &helper) {
  // add report
  if (isReportEnabled()) {
...
void EvalPredSpeedMax::SetGradingMsg(sim_msg::Grading &msg) {
  // set detected event
  EvalHelper::SetDetectedEvent(msg, _detector, _kpi_name);
}

EvalResult EvalPredSpeedMax::IsEvalPass() {
  if (m_KpiEnabled) {
...
bool EvalPredSpeedMax::ShouldStopScenario(std::string &reason) {
