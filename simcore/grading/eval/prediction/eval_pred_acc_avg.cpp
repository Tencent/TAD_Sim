...
#include "eval_pred_acc_avg.h"
#include "trajectory.pb.h"
#include "utils/proto_helper.h"

/**
 * @brief not most effective implement way, needs updating and test with prediciton algorithm, @dhu
 */

namespace eval {
const char EvalPredAccAvg::_kpi_name[] = "Prediction_AccErrorAverage";

sim_msg::TestReport_XlsxSheet EvalPredAccAvg::s_lateral_sheet;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredAccAvg::s_index;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredAccAvg::s_t;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredAccAvg::s_fellow_id;
sim_msg::TestReport_XlsxSheet_SheetData EvalPredAccAvg::s_error;

EvalPredAccAvg::EvalPredAccAvg() { VLOG_0 << "eval algorithm " << _kpi_name << " constructed.\n"; }
bool EvalPredAccAvg::Init(eval::EvalInit &helper) {
  ...
  if (isReportEnabled()) {
    ...
    ReportHelper::ConfigSheetData(s_error, "average acc error[m/s2]");
    ReportHelper::ConfigXLSXSheet(s_lateral_sheet, "average prediction acc error", "");
  }
...
bool EvalPredAccAvg::Step(eval::EvalStep &helper) {
  ...
  if (IsModuleValid() && m_KpiEnabled) {
    ...
    if (pred_msg.GetPayload().size() > 0 && m_pred_payload != pred_msg.GetPayload() &&
    ...
    for (auto fellow_info : fellow_infos) {
      ...
      for (auto i = 0; fellow_info && i < fellow_info->m_actual_traj.size(); ++i) {
        ...
        if (0 <= index && index < fellow_info->m_actual_traj.size()) {
          ...
          // sum error
          avg_error += std::abs(actual_pt->GetAcc().GetX() - pred_pt.GetAcc().GetX());
          counter++;
        }
      }
...
bool EvalPredAccAvg::Stop(eval::EvalStop &helper) {
  // add report
  if (isReportEnabled()) {
...
void EvalPredAccAvg::SetGradingMsg(sim_msg::Grading &msg) {
  // set detected event
  EvalHelper::SetDetectedEvent(msg, _detector, _kpi_name);
}

EvalResult EvalPredAccAvg::IsEvalPass() {
  if (m_KpiEnabled) {
    _case.mutable_info()->set_detected_count(_detector.GetCount());
    if (_detector.GetCount() >= m_Kpi.passcondition().value() && m_Kpi.passcondition().value() >= 0.5) {
      return EvalResult(sim_msg::TestReport_TestState_FAIL, "above prediction acc error");
    } else {
      return EvalResult(sim_msg::TestReport_TestState_PASS, "prediction acc error check pass");
    }
  }

  return EvalResult(sim_msg::TestReport_TestState_PASS, "prediction acc error check skipped");
}
bool EvalPredAccAvg::ShouldStopScenario(std::string &reason) {
  auto ret = _detector.GetCount() >= m_Kpi.finishcondition().value() && m_Kpi.finishcondition().value() >= 0.5;
  if (ret) reason = "above prediction acc error";
