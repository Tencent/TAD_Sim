...
#include "eval_control_msg.h"

namespace eval {
const char EvalControlMsg::_kpi_name[] = "ControlMsgCycle";
sim_msg::TestReport_XYPlot EvalControlMsg::s_cycle_plot;
sim_msg::TestReport_PairData EvalControlMsg::s_counter;

EvalControlMsg::EvalControlMsg() { VLOG_0 << "eval algorithm " << _kpi_name << " constructed.\n"; }
bool EvalControlMsg::Init(eval::EvalInit &helper) {
  ...
  VLOG_1 << "control msg cycle check, topic:" << m_topic << ", cycle time diff[%]:" << m_cycle << "\n";

  return true;
}
bool EvalControlMsg::Step(eval::EvalStep &helper) {
  ...
  if (IsModuleValid() && m_KpiEnabled && m_cycle > 0.0 && !m_topic.empty()) {
    ...
    if (new_msg_arrived) {
      ...
      VLOG_2 << "new control message arrived at " << sim_t_ms << " ms.\n";
    } else {
      // message lost or not yet
      diff_percentage = std::llround(diff) >= std::llround(m_cycle) ? 100.0 : 0.0;
      VLOG_2 << "no new control message arrived.\n";
    }

    if (diff_percentage > m_defaultThreshDouble) m_counter.UpdateCounterBy(1);
    VLOG_2 << "control msg not stable counter " << m_counter.GetCounter() << ", cycle_diff[%]:" << diff_percentage
           << "\n";
...
bool EvalControlMsg::Stop(eval::EvalStop &helper) {
  // add report
  if (isReportEnabled()) {
...
void EvalControlMsg::SetGradingMsg(sim_msg::Grading &msg) {
  // set detected event
  EvalHelper::SetDetectedEvent(msg, _detector, _kpi_name);
}
EvalResult EvalControlMsg::IsEvalPass() {
  if (m_KpiEnabled) {
    _case.mutable_info()->set_detected_count(_detector.GetCount());
    if (_detector.GetCount() >= m_Kpi.passcondition().value() && m_Kpi.passcondition().value() >= 0.5) {
      return EvalResult(sim_msg::TestReport_TestState_FAIL, "above max control msg cycle");
    } else {
      return EvalResult(sim_msg::TestReport_TestState_PASS, "max control msg cycle check pass");
    }
  }

  return EvalResult(sim_msg::TestReport_TestState_PASS, "max control msg cycle check skipped");
}
bool EvalControlMsg::ShouldStopScenario(std::string &reason) {
  auto ret = _detector.GetCount() >= m_Kpi.finishcondition().value() && m_Kpi.finishcondition().value() >= 0.5;
  if (ret) reason = "above max control msg cycle";
