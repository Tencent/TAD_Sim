...
Base::txBool TAD_Manual_AI_VehicleElement::Update(Base::TimeParamManager const& timeMgr) TX_NOEXCEPT {
  if (IsManualMode()) {
    return true;
  } else {
...
sim_msg::Car* TAD_Manual_AI_VehicleElement::FillingElement(txFloat const timeStamp,
                                                           sim_msg::Car* pSimVehicle) TX_NOEXCEPT {
  ClearManualMode();
  return ParentClass::FillingElement(timeStamp, pSimVehicle);
}

void TAD_Manual_AI_VehicleElement::UpdateManualLocation(Base::TimeParamManager const& timeMgr,
  ...
  if (IsAlive()) {
    ...
    GeomCenter().FromWGS84(_location.position().x(), _location.position().y(), _location.position().z());
...
void TAD_Manual_AI_VehicleElement::ComputeScalarVelocity(Base::TimeParamManager const& timeMgr,
...
Base::txBool TAD_Manual_AI_VehicleElement::UpdateManualVehicle(Base::TimeParamManager const& timeMgr) TX_NOEXCEPT {
...
Base::txString TAD_Manual_AI_VehicleElement::ManualDesc() const TX_NOEXCEPT {
