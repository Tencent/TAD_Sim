...
TimeNode& TimeNode::operator=(const TimeParamNode& other) {
  ...
  strTriggerValue = other.strTriggerValue;
...
void CAcceleration::ClearNodes() { m_nodes.clear(); }
...
void CAcceleration::ConvertToTimeStr() {
  ...
  for (; itr != m_nodes.end(); ++itr) {
    if (itr->nTriggerType == TCT_TIME_ABSOLUTE) {
      ...
      std::string strAcc = CUtils::doubleToStringDot3((*itr).dAcc);
      ...
      std::string strEndValue = CUtils::doubleToStringDot3((*itr).dStopValue);
