...
int CGenerator::Generate() {
  ...
  size_t nVelocity = (pp.m_fStartVelocityEnd - pp.m_fStartVelocityStart) / pp.m_fStartVelocitySep;

  for (size_t i = 0; i <= nVelocity; i++) {
    float fVelocity = pp.m_fStartVelocityStart + i * pp.m_fStartVelocitySep;
    char strVelocityBuf[256] = {0};
    sprintf(strVelocityBuf, "%4f", fVelocity);

    CSimulation s(m_simParam);
    s.m_planner = pp;
    s.m_planner.m_strStartVelocity = strVelocityBuf;
...
int CGenerator::CalculateCount(int64_t& count, int64_t& size) {
  ...
  int nVelocity = (pp.m_fStartVelocityEnd - pp.m_fStartVelocityStart) / pp.m_fStartVelocitySep;

  count = (nVelocity + 1) * llCount;
  size = llSize;
...
int CGenerator::Parse(const char* strFileName, sTagBatchSceneParam& param, const char* strParams) {
  ...
  m_simParam.m_plannerparam.m_fStartVelocityStart = param.m_ego_velocity_start;
  m_simParam.m_plannerparam.m_fStartVelocityEnd = param.m_ego_velocity_end;
  m_simParam.m_plannerparam.m_fStartVelocitySep = param.m_ego_velocity_sep;
...
int CGenerator::GenerateCount(const char* strSimName, sTagBatchSceneParam& param, uint64_t& ullCount, uint64_t& ullSize,
  ...
  ullCount = (nVelocity + 1) * ullTrafficCount;
