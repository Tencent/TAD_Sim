...
void CAABB::Reset() {
  m_min = CVector3(INFINITY, INFINITY, INFINITY);
  m_max = CVector3(-INFINITY, -INFINITY, -INFINITY);
}

void CAABB::Set(const CVector3& min, const CVector3& max) {
  m_min = min;
  m_max = max;
}
...
void CAABB::Expand(const CVector3* pPoints, int nNum) {
  if (!pPoints || nNum < 1) {
    return;
  }
...
CVector3 CAABB::Center() { return m_max * 0.5 + m_min * 0.5; }
