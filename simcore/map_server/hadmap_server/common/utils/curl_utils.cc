...
#include "security/openssl_util.h"
...
EasyCurl::EasyCurl() {
  ...
  std::call_once(once, []() { CHECK_EQ(0, curl_global_init(CURL_GLOBAL_DEFAULT & ~CURL_GLOBAL_SSL)); });
  curl_ = curl_easy_init();
  CHECK(curl_) << "Could not init curl";
}

EasyCurl::~EasyCurl() { curl_easy_cleanup(curl_); }

Status EasyCurl::FetchURL(const string& url, std::string* dst, const vector<string>& headers) {
  return DoRequest(url, nullptr, dst, headers);
}

Status EasyCurl::PostToURL(const string& url, const string& post_data, std::string* dst) {
  return DoRequest(url, &post_data, dst);
}

Status EasyCurl::DoRequest(const string& url, const string* post_data, std::string* dst,
                           const vector<string>& headers) {
  CHECK_NOTNULL(dst)->clear();

  if (!verify_peer_) {
    RETURN_NOT_OK(TranslateError(curl_easy_setopt(curl_, CURLOPT_SSL_VERIFYHOST, 0)));
    RETURN_NOT_OK(TranslateError(curl_easy_setopt(curl_, CURLOPT_SSL_VERIFYPEER, 0)));
  }

  if (use_spnego_) {
    RETURN_NOT_OK(TranslateError(curl_easy_setopt(curl_, CURLOPT_HTTPAUTH, CURLAUTH_NEGOTIATE)));
    ...
    RETURN_NOT_OK(TranslateError(curl_easy_setopt(curl_, CURLOPT_USERPWD, ":")));
  }

  if (verbose_) {
    RETURN_NOT_OK(TranslateError(curl_easy_setopt(curl_, CURLOPT_VERBOSE, 1)));
  }

  // Add headers if specified.
  struct curl_slist* curl_headers = nullptr;
  auto clean_up_curl_slist = MakeScopedCleanup([&]() { curl_slist_free_all(curl_headers); });
  ...
  RETURN_NOT_OK(TranslateError(curl_easy_setopt(curl_, CURLOPT_WRITEDATA, static_cast<void*>(dst))));
  if (post_data) {
  ...
  if (!custom_method_.empty()) {
    RETURN_NOT_OK(TranslateError(curl_easy_setopt(curl_, CURLOPT_CUSTOMREQUEST, custom_method_.c_str())));
  }
  ...
  if (timeout_.Initialized()) {
    ...
    RETURN_NOT_OK(TranslateError(curl_easy_setopt(curl_, CURLOPT_TIMEOUT_MS, timeout_.ToMilliseconds())));
  }
  ...
  if (val != 200) {
    return Status::RemoteError(strings::Substitute("HTTP " + std::to_string(val)));
  }
  return Status::OK();
}
