...
static void addGfxCmdScissor(int x, int y, int w, int h) {
  ...
  cmd.col = 0;
  cmd.rect.x = static_cast<int16_t>(x), cmd.rect.y = static_cast<int16_t>(y), cmd.rect.w = static_cast<int16_t>(w),
  cmd.rect.h = static_cast<int16_t>(h),
}
...
static void addGfxCmdRect(float x, float y, float w, float h, unsigned int color) {
  ...
  cmd.flags = 0;
  cmd.col = color;
  cmd.rect.x = static_cast<int16_t>((x * 8.0f)), cmd.rect.y = static_cast<int16_t>((y * 8.0f)),
  cmd.rect.w = static_cast<int16_t>((w * 8.0f)), cmd.rect.h = static_cast<int16_t>((h * 8.0f)), cmd.rect.r = 0;
}
...
static void addGfxCmdLine(float x0, float y0, float x1, float y1, float r, unsigned int color) {
  ...
  cmd.flags = 0;
  cmd.col = color;
  cmd.line.x0 = static_cast<int16_t>((x0 * 8.0f)), cmd.line.y0 = static_cast<int16_t>((y0 * 8.0f)),
  cmd.line.x1 = static_cast<int16_t>((x1 * 8.0f)), cmd.line.y1 = static_cast<int16_t>((y1 * 8.0f)),
  cmd.line.r = static_cast<int16_t>((r * 8.0f)),
}
...
static void addGfxCmdRoundedRect(float x, float y, float w, float h, float r, unsigned int color) {
  ...
  cmd.col = color;
  cmd.rect.x = static_cast<int16_t>((x * 8.0f)), cmd.rect.y = static_cast<int16_t>((y * 8.0f));
  cmd.rect.w = static_cast<int16_t>((w * 8.0f)), cmd.rect.h = static_cast<int16_t>((h * 8.0f)),
  cmd.rect.r = static_cast<int16_t>((r * 8.0f)),
}
...
static void addGfxCmdText(int x, int y, int align, const char* text, unsigned int color) {
  ...
  cmd.flags = 0;
  cmd.col = color;
  cmd.text.x = static_cast<int16_t>(x), cmd.text.y = static_cast<int16_t>(y),
  cmd.text.align = static_cast<int16_t>(align), cmd.text.text = allocText(text);
}
...
bool imguiBeginScrollArea(const char* name, int x, int y, int w, int h, int* scroll) {
  ...
  addGfxCmdRoundedRect(static_cast<float>(x), static_cast<float>(y), static_cast<float>(w), static_cast<float>(h), 6,
                       imguiRGBA(0, 0, 0, 192));
...
void imguiEndScrollArea() {
  ...
  float barHeight = static_cast<float>(h) / static_cast<float>(sh);
  ...
  if (barHeight < 1) {
    ...
    int hx = x;
    int hy = y + static_cast<int>(barY * h);
    int hw = w;
    int hh = static_cast<int>(barHeight * h);
    ...
    // BG
    addGfxCmdRoundedRect(static_cast<float>(x), static_cast<float>(y), static_cast<float>(w), static_cast<float>(h),
                         static_cast<float>(w) / 2 - 1, imguiRGBA(0, 0, 0, 196));
    // Bar
    if (isActive(hid))
      addGfxCmdRoundedRect(static_cast<float>(hx), static_cast<float>(hy), static_cast<float>(hw),
                           static_cast<float>(hh), static_cast<float>(w) / 2 - 1, imguiRGBA(255, 196, 0, 196));
    else
      addGfxCmdRoundedRect(static_cast<float>(hx), static_cast<float>(hy), static_cast<float>(hw),
                           static_cast<float>(hh), static_cast<float>(w) / 2 - 1,
...
bool imguiButton(const char* text, bool enabled) {
  ...
  addGfxCmdRoundedRect(static_cast<float>(x), static_cast<float>(y), static_cast<float>(w), static_cast<float>(h),
                       static_cast<float>(BUTTON_HEIGHT) / 2 - 1, imguiRGBA(128, 128, 128, isActive(id) ? 196 : 96));
  if (enabled)
...
bool imguiItem(const char* text, bool enabled) {
  ...
  if (isHot(id))
    addGfxCmdRoundedRect(static_cast<float>(x), static_cast<float>(y), static_cast<float>(w), static_cast<float>(h),
                         2.0f, imguiRGBA(255, 196, 0, isActive(id) ? 196 : 96));
...
bool imguiCheck(const char* text, bool checked, bool enabled) {
  ...
  addGfxCmdRoundedRect(static_cast<float>(cx) - 3, static_cast<float>(cy) - 3, static_cast<float>(CHECK_SIZE) + 6,
                       static_cast<float>(CHECK_SIZE) + 6, 4, imguiRGBA(128, 128, 128, isActive(id) ? 196 : 96));
  if (checked) {
    if (enabled) {
      addGfxCmdRoundedRect(static_cast<float>(cx), static_cast<float>(cy), static_cast<float>(CHECK_SIZE),
                           static_cast<float>(CHECK_SIZE), static_cast<float>(CHECK_SIZE) / 2 - 1,
...
bool imguiSlider(const char* text, float* val, float vmin, float vmax, float vinc, bool enabled) {
  ...
  addGfxCmdRoundedRect(static_cast<float>(x), static_cast<float>(y), static_cast<float>(w), static_cast<float>(h), 4.0f,
                       imguiRGBA(0, 0, 0, 128));
  ...
  if (isActive(id))
    addGfxCmdRoundedRect(static_cast<float>(x + m), static_cast<float>(y), static_cast<float>(SLIDER_MARKER_WIDTH),
                         static_cast<float>(SLIDER_HEIGHT), 4.0f, imguiRGBA(255, 255, 255, 255));
  else
    addGfxCmdRoundedRect(static_cast<float>(x + m), static_cast<float>(y), static_cast<float>(SLIDER_MARKER_WIDTH),
                         static_cast<float>(SLIDER_HEIGHT), 4.0f,
                         isHot(id) ? imguiRGBA(255, 196, 0, 128) : imguiRGBA(255, 255, 255, 64));

  int digits = static_cast<int>(ceilf(log10f(vinc)));
  char fmt[16];
...
void imguiSeparatorLine() {
  ...
  addGfxCmdRect(static_cast<float>(x), static_cast<float>(y), static_cast<float>(w), static_cast<float>(h),
