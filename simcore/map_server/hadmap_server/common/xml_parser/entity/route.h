...
struct sPathPoint {
  ...
  std::string m_strAcc;
  std::string m_strHead = "0.0";
  std::string m_strFrontwheel;
  std::string m_strDriver;
  sPathPoint();
  sPathPoint(std::string lon, std::string lat, std::string alt);
  void Reset();
  void ConvertToStr();
  void ConvertToValue();
};
...
class CRoute {
 public:
  ...
  void ModifyStart(double dStartLon, double dStartLat);
  ...
  //
  double getStartLon() const { return std::atof(m_strStartLon.c_str()); }

  double getStartLat() const { return std::atof(m_strStartLat.c_str()); }
