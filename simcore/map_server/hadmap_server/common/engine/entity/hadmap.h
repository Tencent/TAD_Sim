...
class CHadmap {
 public:
  ...
  int QueryLonLatByPoint(double dLon, double dLat, lanepkid laneID, double dShift, double dOffset, double& dFinalLon,
                         double& dFinalLat, double& dFinalAlt, double& dWidth, double& dFinalShift,
                         bool isStart = false);
  ...
  //
  int getLaneLink(const double lon, const double lat, int& roadid, std::vector<linkData>& linkdatas);

  hadmap::txMapHandle* getHandle();
  ...
  std::uintmax_t FileSize() { return m_file_size; }
  ...
  void SetFileSize(std::uintmax_t size) { m_file_size = size; }
