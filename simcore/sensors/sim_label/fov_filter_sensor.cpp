 ...
 */
void FovFilterSensor::setCarRotation(double roll, double pitch, double yaw) {
  Eigen::Quaterniond q = Eigen::AngleAxisd(yaw, Eigen::Vector3d::UnitZ()) *
                         Eigen::AngleAxisd(pitch, Eigen::Vector3d::UnitY()) *
                         Eigen::AngleAxisd(roll, Eigen::Vector3d::UnitX());
...
/**
 * @brief calculate the local position of the car
 *
 * @param wp vector in world coordinate
 * @return local vector of car
 */
Eigen::Vector3d FovFilterSensor::FovVectorOnlyCar(const Eigen::Vector3d &wp) const { return R * (wp + T); }
 ...
 *
 * @param roll roll angle and returned
 * @param pitch pitch angle and returned
 * @param yaw yaw angle and returned
 */
