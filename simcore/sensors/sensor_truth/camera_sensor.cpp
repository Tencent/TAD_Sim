 ...
 * @return true if it's inside FOV
 * @return false if it's outside FOV
 */
bool CameraSensor::inFov(const Eigen::Vector3d &wp, double distance) const {
  Eigen::Vector3d c = Rl * (R * (wp + T) + Tl);
  if (c.z() <= 0 || c.z() > distance) {
    return false;
  }
  Eigen::Vector2d uv = world2pixel(wp);
  return uv.x() > 0 && uv.x() < w && uv.y() > 0 && uv.y() < h;
...
Eigen::Vector2d CameraSensor::world2pixel(const Eigen::Vector3d &wp) const {
  ...
  if (k1 != 0) {
    ...
    double r4 = r2 * r2;
    double r6 = r2 * r4;
    double dx = x * (1 + k1 * r2 + k2 * r4 + k3 * r6) + 2 * p1 * x * y +
                p2 * (r2 + 2 * x * x);
    double dy = y * (1 + k1 * r2 + k2 * r4 + k3 * r6) + 2 * p2 * x * y +
                p1 * (r2 + 2 * y * y);
